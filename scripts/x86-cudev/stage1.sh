...
#   version, since someone had the brilliant idea to put them all into a single
#   repo.
echo "Package: libnvinfer* libnvonnxparsers*" > /etc/apt/preferences.d/cuda-repository-pin
echo "Pin: version *+cuda12.8*" >> /etc/apt/preferences.d/cuda-repository-pin
...
# Instal TensorRT with full Python and C++ support
# NOTES
# - To achieve this, according to
#   https://docs.nvidia.com/deeplearning/tensorrt/latest/installing-tensorrt/installing.html#debian-installation
#   apt is the best installation method.
apt-get update
apt-get install -y --no-install-recommends \
...
cd /opt
/opt/scripts/install_rust.sh x86-cudev 1.85.0
...
cd /opt
/opt/scripts/install_nlohmann_json.sh x86-cudev 3.12.0
...
# Install ROS 2
cd /opt
/opt/scripts/install_ros2.sh x86-cudev jazzy 8294968ee233629c8fb223b2a29bb93ccce7480a
...
# Install Gazebo
cd /opt
/opt/scripts/install_gazebo.sh x86-cudev harmonic jazzy
