...
if [ "$BASE_UNIT" = "jetson5" ]; then
  # Get version
  ORT_VERSION="1.12.1"

  echo "Building ONNX Runtime $ORT_VERSION for $BASE_UNIT ..."
  ...
  pip install ./build/Linux/Release/dist/onnxruntime_gpu-${ORT_VERSION}-cp38-cp38-linux_aarch64.whl
  cd /opt
